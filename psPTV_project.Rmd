...
all_vars$pred_class_b = all_vars$prediction_prob_b > 0.245
all_vars$violator_class = all_vars$violator == 'yes'
...
var_data$pred_class_b = var_data$prediction_prob_b > 0.245
...
vars_non_true = all_vars[all_vars$RF_class_50 %in% c(FALSE, NA), ]
...
table(random_gene_data$no_fin_asj > 1e-4 & random_gene_data$no_fin_asj_prior < 1e-5)
...
vars_pext_flt = all_vars[!(all_vars$pext_avg < 0.1), ]
cov_data = read.table('./covered_genes.tsv', header=T, sep='\t')
...
clv_data = read.table('clv_ptv_ready.tsv', sep='\t', header=T)
...
clv_data$RF_class_50 = clv_data$prediction_prob_b > 0.50
...
patho = read.table('clv.path.tsv', sep='\t', header=T)
benign = read.table('clv.benign.tsv', sep='\t', header=T)
patho$benign = 'no'
benign$benign = 'yes'
...
all_vars$RF_class_50_c = all_vars$pred_all_data > 0.5
...
It's a bit awkward that in the genes with low conservation level the overall proportion of variants classified as lcpLoF is slightly lower for violators than for non-violators, but it's great that the proportion of variants classified as lcpLoF for confident ClinVar pathogenic variants is lower than either of them (the differnece is clear and sharp for RF_class_25_c). However, the classifier's behavior is bad given that lots of ClinVar confident pathogenic variants are still fitered out (in contrast to the "b" model). Hence, we stick with the balanced model.
