...
vars_non_true = all_vars[all_vars$pred_all_data < 0.4 |
                           is.na(all_vars$pred_all_data), ]
gene_level = aggregate(AC_main ~ gene, vars_non_true, sum)
gene_level$source_ac = sapply(as.character(gene_level$gene), function(x) cpl_head[x, 'AC_main'])
...
ggplot(ttl_stats, aes(x=s_bin, y=PointEst, fill=violator)) + 
  ...
  scale_y_continuous(limits=c(0, 1)) + 
  scale_fill_manual(values = c("#d2b12bff", "#5392f8ff", 
                               "#fbbab5ff", "#7fdfe1ff"))
```
...
vars_non_true = all_vars[all_vars$RF_class_50_d %in% c(FALSE, NA), ]
gene_level = aggregate(AC_main ~ gene, vars_non_true, sum)
gene_level$source_ac = sapply(as.character(gene_level$gene), function(x) cpl_head[x, 'AC_main'])
...
vars_pext_flt = all_vars[!(all_vars$pext_avg < 0.35), ]
cov_data = read.table('./covered_genes.tsv', header=T, sep='\t')
...
I also want to plot a total ROC curve.

```{r combineroc, warning=FALSE}
all_vars$violator = as.factor(all_vars$violator)
train_ind = sample(1:nrow(all_vars), 0.9*nrow(all_vars))
trainset = all_vars[train_ind, ]
include = as.character(trainset$violator) == 'yes' | runif(nrow(trainset)) < 0.22
trainset_balanced = trainset[include, ]
testset = all_vars[-train_ind, ]

testset$prediction_a = predict(my_forest_b, testset, type='vote')[, 2]
testset$prediction_b = predict(my_forest_c, testset, type='vote')[, 2]

a_cons_set = testset[testset$s_main > 0.02, ]

pROC_obj_a <- roc(testset$violator, testset$prediction_a,
                smoothed = TRUE,
                # arguments for ci
                ci=TRUE, ci.alpha=0.9, stratified=FALSE,
                # arguments for plot
...
pROC_obj_b <- roc(testset$violator, testset$prediction_b,
                smoothed = TRUE,
                # arguments for ci
                ci=TRUE, ci.alpha=0.9, stratified=FALSE,
                # arguments for plot
...
pROC_obj_c <- roc(testset$violator, testset$pext_avg,
                smoothed = TRUE,
                # arguments for ci
                ci=TRUE, ci.alpha=0.9, stratified=FALSE,
                # arguments for plot
...
pROC_obj_d <- roc(a_cons_set$violator, a_cons_set$prediction_a,
                smoothed = TRUE,
                # arguments for ci
                ci=TRUE, ci.alpha=0.9, stratified=FALSE,
                # arguments for plot
...
pROC_obj_e <- roc(a_cons_set$violator, a_cons_set$prediction_b,
                smoothed = TRUE,
                # arguments for ci
                ci=TRUE, ci.alpha=0.9, stratified=FALSE,
                # arguments for plot
...
pROC_obj_f <- roc(a_cons_set$violator, a_cons_set$pext_avg,
                smoothed = TRUE,
                # arguments for ci
                ci=TRUE, ci.alpha=0.9, stratified=FALSE,
                # arguments for plot
...
roc_curves = data.frame(sens = c(pROC_obj_a$sensitivities,
                                 pROC_obj_b$sensitivities,
                                 pROC_obj_c$sensitivities,
                                 pROC_obj_d$sensitivities,
                                 pROC_obj_e$sensitivities,
                                 pROC_obj_f$sensitivities),
                        spec = c(pROC_obj_a$specificities,
                                 pROC_obj_b$specificities,
                                 pROC_obj_c$specificities,
                                 pROC_obj_d$specificities,
                                 pROC_obj_e$specificities,
                                 pROC_obj_f$specificities),
                        model = c(rep('A', length(pROC_obj_a$sensitivities)),
                                  rep('B', length(pROC_obj_b$sensitivities)),
                                  rep('C', length(pROC_obj_c$sensitivities)),
                                  rep('A', length(pROC_obj_d$sensitivities)),
                                  rep('B', length(pROC_obj_e$sensitivities)),
                                  rep('C', length(pROC_obj_f$sensitivities))),
                        set = c(rep('all', length(pROC_obj_a$sensitivities) + 
                                      length(pROC_obj_b$sensitivities) +
                                      length(pROC_obj_c$sensitivities)),
                                rep('cons', length(pROC_obj_d$sensitivities) + 
                                      length(pROC_obj_e$sensitivities) +
                                      length(pROC_obj_f$sensitivities))))
...
ggplot(roc_curves, aes(x=spec, y=sens, col=model)) + geom_line(lwd=1.4) +
  geom_area(aes(group = model, fill=model), col=NA,
              alpha = 0.35, position = 'identity') + theme_bw() +
  theme(plot.grid=element_blank()) +
  scale_x_reverse() + guides(fill=F, col=F) +
  xlab('Specificity') + ylab('Sensitivity') +
  facet_grid(vars(model), vars(set)) +
  scale_color_manual(values=c(mycol1, mycol2, mycol3)) +
  scale_fill_manual(values=c(mycol1, mycol2, mycol3))
```

And let's create the plot with AUCs of the gnomAD vs ClinVar model. 

```{r aucs, warning=FALSE}
aucs = data.frame(model = factor(c('pext', 'gnC', 'gnA', 'clv'),
                                 levels=c('pext', 'gnC', 'gnA', 'clv')),
                  auc = c(0.550, 0.549, 0.674, 0.781),
                  lower = c(0.509, 0.507, 0.625, 0.709),
                  upper = c(0.590, 0.592, 0.722, 0.854))

a <- ggplot(aucs, aes(x=model, y=auc, fill=model)) + 
  geom_bar(stat='identity', col='black') +
  geom_errorbar(aes(ymin=lower, ymax=upper, width=0.4)) +
  theme_bw() + theme(panel.grid = element_blank()) +
  scale_fill_manual(values=c(mycol3, mycol1, mycol2, mycol6))

print(a)
```

Finally, we have to create a plot with the percentages of genes with PTV count distribution non-uniformity (this goes to Fig. 2f).

```{r makegenecounts, warning=FALSE}
get_value <- function(df){
  cov_data = read.table('./covered_genes.tsv', header=T, sep='\t')

  df = df[as.character(df$gene) %in% as.character(cov_data$name), ]
  df$gene = droplevels(df$gene)
  
  ac_cols = grepl('AC_', colnames(df))
  an_cols = grepl('AN_', colnames(df))
  
  gene_data = data.frame(gene = sort(unique(df$gene)))
  for (i in colnames(df)[ac_cols]){
    gene_data[, i] = as.numeric(by(df[, i], df$gene, sum))
  }
  gene_data$AC_main = gene_data$AC_nfe + gene_data$AC_afr + gene_data$AC_amr + 
    gene_data$AC_eas + gene_data$AC_sas
  for (i in colnames(df)[an_cols]){
    gene_data[, i] = as.numeric(by(df[, i], df$gene, mean))
  }
  gene_data$AN_main = gene_data$AN_nfe + gene_data$AN_afr + gene_data$AN_amr + 
    gene_data$AN_eas + gene_data$AN_sas
  ...
  return(binconf(sum(flt_gene_data$no_fin_asj >= 1e-5 & 
              flt_gene_data$no_fin_asj_prior < 1e-5, na.rm=T), 
          sum(flt_gene_data$no_fin_asj_prior < 1e-5, na.rm=T)))
  
}

rands <- get_value(all_vars[sample(1:nrow(all_vars), 92000),])
pexts <- get_value(all_vars[all_vars$pext_avg > 0.6 |
                              is.na(all_vars$pext_avg),])
gna <- get_value(all_vars[all_vars$pred_all_data < 0.25 |
                     is.na(all_vars$pred_all_data),])
clv <- get_value(all_vars[all_vars$prediction_d <= 0.325 |
                            is.na(all_vars$prediction_d),])

removal = as.data.frame(sum(flt_gene_data$no_fin_asj_prior <= 1e-5, na.rm=T) -
                  sum(flt_gene_data$no_fin_asj_prior <= 1e-5, na.rm=T) * 
                    rbind(rands, pexts, gna, clv))
removal$model = c('random', 'pext', 'gna', 'clv')
removal[5, ] = c(sum(flt_gene_data$no_fin_asj_prior <= 1e-5, na.rm=T), NA,
                 NA, 'noflt')
colnames(removal) = c('PointEst', 'Upper', 'Lower', 'model')
removal$model = factor(removal$model, levels=c('noflt', 'random',
                                               'pext', 'gna', 'clv'))
removal$PointEst = as.numeric(as.character(removal$PointEst))
removal$Lower = as.numeric(as.character(removal$Lower))
removal$Upper = as.numeric(as.character(removal$Upper))

b <- ggplot(removal, aes(x=model, y=PointEst, fill=model)) + 
  geom_bar(stat='identity', col='black') +
  geom_errorbar(aes(ymin=Lower, ymax=Upper, width=0.4)) +
  theme_bw() + theme(panel.grid = element_blank()) +
  scale_fill_manual(values=c('white', 'gray', mycol3, mycol2, mycol6))

plot_grid(a, b, nrow=1, rel_widths = c(0.8, 1))
```
