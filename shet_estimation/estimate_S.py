...
df = pd.read_csv('full_out_IG.tsv')
...
def estimateS(n, N, U, a, b):
    ...
    true_likelihoods = [x for x in likelihoods if ((x[1] is not None) and (not np.isnan(x[1])))]
    S = true_likelihoods[int(np.argmax([x[1] for x in true_likelihoods]))]
    return S
...
df.to_csv('full_corr_wshet.tsv', index=False)
